from app.models import db, Cart, Item, User, Inventory, Category
from app.exceptions.itemDoesNotExists import ItemDoesNotExists
...
def add_cart_item(item_id: int, user_id: int) -> None:
    if not check_item_existence(item_id):
        raise ItemDoesNotExists(f"Item with {item_id} does not exists in our database.")
